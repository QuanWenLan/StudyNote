...
root@dk-server-webpush-service-6f54b47958-jmkw8:/opt# ps -eww 14300
 PID TTY      STAT   TIME COMMAND
 1 ?        Ssl  112:51 java -Djava.security.egd=file:/dev/./urandom -Xms512m -Xmx512m -Dlogging.file.path=/logs -Dlogging.path=/logs -Dspring.profiles.active=test -Dserver.port=8001 -Dnacos.config.server-addr=10.200.0.15 -Dnacos.config.namespace=dk-test-5 -Dnacos.config.username=dk_ro -Dnacos.config.password=Dk_Ro893*% -Djava.security.egd=file:/dev/./urandom -jar app.jar
```
...
![image-20240415143945618](media/images/image-20240415143945618.png)

如果想查看每个核的消耗情况，先进入top视图，然后 按1:

![image-20240415144011369](media/images/image-20240415144011369.png)

我们还可以通过 top -Hp pid 查看具体线程使用系统资源情况：` top -Hp 28886`

![image-20240416160840683](media/images/image-20240416160840683.png)
...
![image-20240415144709755](media/images/image-20240415144709755.png)
