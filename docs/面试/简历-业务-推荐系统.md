...
最后这边的推荐会返回2类商品，也就是首页的营养补充品和Greendos商品。推荐出来的是商品的product_num，随后还需要根据这个num去找到对应的产品之后再返回。

###### 产品详情页面推荐

![image-20231209152302659](media/images/image-20231209152302659.png)

同样类似的会发一个请求，也是会走到上面的截图位置。从发送的请求中可以看出，form.getClientId()是一个null，因此这边会传递一个“--” 到接口中。

![image-20231209153023461](media/images/image-20231209153023461.png)

然后走到这个请求接口：itemId是productId， customerId 则是 clientId，webUUID则是当前登录的uuid。

![image-20231209152808498](media/images/image-20231209152808498.png)

随后方法中会读取对应的文件作为数据：categoryProductData.txt，记住这里是不需要产品的偏好值的。

![image-20231209153409481](media/images/image-20231209153409481.png)

从代码中可看到，如果传递的clientid为 “--” 会做一些处理。随后创建推荐代码进行相应的推荐。使用的是基于物品的Recommender。

```java
//2.创建similar相似度， 选择了 TANIMOTO 相似性的度量方式
ItemSimilarity similarity = RecommendFactory.itemSimilarity(RecommendFactory.SIMILARITY.TANIMOTO, dataModel);
//4.构建推荐器方式一，通过true,false去控制使用哪个物品推荐器，recommend
/*Recommender recommender = RecommendFactory
      .itemRecommender(similarity,true)
      .buildRecommender(dataModel);

       //5.基于构建推荐器方式一的推荐，推荐给当然用户的5个商品
       List<RecommendedItem> recommendedItems = recommender.recommend(curClientItem.getClientNum(), 5);*/

//4.构建推荐器方式二，直接用GenericItemBasedRecommender或GenericBooleanPrefItemBasedRecommender推荐器，直接用的优点是这两个推荐器有实现对客户基于指定产品的推荐
GenericItemBasedRecommender recommender2 = new GenericItemBasedRecommender(dataModel, similarity);
//指定要排除的产品，包括没货和下架的产品
List<ProductIdNum> productsToExclude = baseMapper.listAllSuOrSelloutProduct();
//增加排除当前产品erp设置的相关产品
//List<ProductIdNum> relaPdToExclude = baseMapper.listRelatedProductByItem(curProductItem.getProductId());
//productsToExclude.addAll(relaPdToExclude);
Set productIdsToExclude = getProduct(productsToExclude);
Rescorer rescorer = new ProductRescorer(productIdsToExclude);
...
excludeProductIds.add(curProductItem.getProductId());
try {
    //5.基于构建推荐器方式一的推荐，基于当前产品推荐给当然用户的5个商品
    //List<RecommendedItem> recommendedItems = recommender2.recommendedBecause(curClientItem.getClientNum(), curProductItem.getProductNum(), 5);
    // 进行推荐
    recommendedItems = recommender2.mostSimilarItems(curProductItem.getProductNum(), 6, rescorer);
    List<String> productIdNums = recommendedItems.stream()
        .map(item -> String.valueOf(item.getItemID()))
        .collect(Collectors.toList());
    List<ProductIdNum> productIdNumList = CollectionUtils.isEmpty(productIdNums) ? new ArrayList<>() : baseMapper.getProductIdNum(productIdNums);
    //得到推荐商品的自增编号，先转换成产品编号再返回
    products = recommendedItems.stream().map(pro -> {
        ProductIdNum respItem = productIdNumList.stream().filter(pre -> {
            return Objects.equals(pre.getProductNum(), pro.getItemID());
        }).findAny().orElse(null);
        Product product = new Product();
        if (respItem != null) {
            product.setProductId(respItem.getProductId());
            product.setProductName(respItem.getProductName());
            product.setWebProductId(respItem.getWebProductId());
            product.setQty(respItem.getQty());
            product.setStatus(respItem.getStatus());
        }else{
            product.setProductId(String.valueOf(pro.getItemID()));
        }
        return product;
    }).collect(Collectors.toList());
...
// 下面还有一些，如果推荐数量不足的话，则补足销售量高的产品，并记录下这些推荐的数据。
```

###### 购物车页面推荐

请求接口代码：使用到了文件clientProductData.txt，记录了用户购买、浏览、收藏的一些偏好值的数据。

![image-20231209152542061](media/images/image-20231209152542061.png)

同样可以看具体的逻辑：

```java
ClientItemResponse response = new ClientItemResponse();
try {
   log.info("start recommend for client(Cart):" + clientId);
   FileDataModel dataModel = new FileDataModel(new File(cliPdFilepath));
   LongPrimitiveIterator userIDs = dataModel.getUserIDs();
   //参数传递的是字符类型客户id，需要先取该客户id的客户自增编号再赋值计算
   ClientIdNum curItem = null;
   if ("--".equals(clientId) && !"--".equals(webUUID)) {
      List<ClientIdNum> clientByUUid = baseMapper.getClientIdByUUid(webUUID);
      if (clientByUUid.size() > 0) {
         List<ClientIdNum> resultList = baseMapper.getClientIdNumByClientId(clientByUUid.get(0).getClientId());
         curItem = CollectionUtils.isEmpty(resultList) ? null : resultList.get(0);;
      }
   } else {
      List<ClientIdNum> resultList = baseMapper.getClientIdNumByClientId(clientId);
      curItem = CollectionUtils.isEmpty(resultList) ? null : resultList.get(0);;
   }
   if (curItem == null) {
      //无法知晓客户id，则推荐6销量好的产品
      List<ProductIdNum> pds = baseMapper.getSellRankingProduct(6, null);
      List<Product> products = new ArrayList<>();
      if (pds != null) {
         for (ProductIdNum productIdNum : pds) {
            Product product = new Product();
            product.setProductId(productIdNum.getProductId());
            products.add(product);
         }
      }
      String prodcutIds = products.stream().map(pd -> pd.getProductId()).collect(Collectors.joining("', '", "'", "'"));
      response.setPoducts(products);
      response.setProductIds(prodcutIds);

      log.info("End recommend and unknow client id");
      response.setReturnCode("0");
      return response;
   }
    // 。。。省略一些逻辑
    //获取子产品不够的组合产品集
    List<ProductIdNum> noEnoughProductKitToExclude = baseMapper.getProductKitNoEnough();
    List<String> noEnoughProductKitProductIds = CollectionUtils.isEmpty(noEnoughProductKitToExclude) ? new ArrayList<>() :
    noEnoughProductKitToExclude.stream().map(ProductIdNum::getProductId).collect(Collectors.toList());

    //获取购物车集合
    List<ProductIdNum> shoppingCartToExclude = baseMapper.getShoppingCartByClientId(curItem.getClientId());
    List<String>  shoppingCartProductIds = CollectionUtils.isEmpty(shoppingCartToExclude) ? new ArrayList<>() :
    shoppingCartToExclude.stream().map(ProductIdNum::getProductId).collect(Collectors.toList());

    //初始化需要排除的productIds
    List<String> excludeProductIds = new ArrayList<>();
    excludeProductIds.addAll(noEnoughProductKitProductIds);
    excludeProductIds.addAll(shoppingCartProductIds);


    List<ProductIdNum> addPds = new ArrayList<>();
    List<Product> products = new ArrayList<>();
    boolean clientInDataFile = hasClientIdInDataFile(userIDs, curItem.getClientNum());
    if (clientInDataFile != true) {
        //客户在数据中存在，但基础数据未有记录，即新注册的用户还未有任何操作
        addPds = baseMapper.getSellRankingProduct(6, excludeProductIds);
        for (ProductIdNum productIdNum : addPds) {
            Product product = new Product();
            product.setProductId(productIdNum.getProductId());
            products.add(product);
        }
        ...
        int thr = (r.nextInt(5) + 1) * 2;
        double threshold = (double) thr / 10;
        ThresholdUserNeighborhood neighborhood = new ThresholdUserNeighborhood(threshold, similarity, dataModel);
        //clientId的近邻对象都有谁
        long[] userNeighborhood = neighborhood.getUserNeighborhood(curItem.getClientNum());
        //4.构建推荐器recommend，并指定要排除的product id
        List<ProductIdNum> productsToExclude = baseMapper.listAllSuOrSelloutProduct();
        Set productIdsToExclude = getProduct(productsToExclude);
        IDRescorer rescorer = new ProductIDRescorer(productIdsToExclude);
        ...
        //排除指定产品后，展示类似的6个商品
        List<RecommendedItem> recommendedItems = recommender.recommend(curItem.getClientNum(), 6, rescorer);
        //...
} catch (Exception e) {
    response.setReturnCode("1");
    log.error("Failed recommend  (Cart) :", e);
}
```

其实大部分都是类似的一个操作，最终的目的是要获取到product_num以此来获取product，不够则补充销量高的产品。
