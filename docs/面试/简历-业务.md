...
CREATE TABLE `promotions` (
  ...
  `from_date` datetime DEFAULT NULL, -- 时间条件
  ...
  `target_product_id` varchar(200) COLLATE utf8mb4_bin DEFAULT NULL, -- 产品组编号
  `target_client_id` varchar(50) COLLATE utf8mb4_bin DEFAULT NULL, -- 客户类别
  `use_once` tinyint(1) NOT NULL DEFAULT '0',
  `one_per_client` tinyint(1) NOT NULL DEFAULT '0', -- 是否每个客户只用一次
  `only_first_order` tinyint(1) NOT NULL DEFAULT '0', -- 是否是首单
  `coupon` tinyint(1) NOT NULL DEFAULT '0', -- 是否是优惠券
  `channel` varchar(5) COLLATE utf8mb4_bin DEFAULT NULL, -- 
  `store_id` varchar(50) COLLATE utf8mb4_bin DEFAULT NULL, -- 商店条件
  ...
  `mon` tinyint(1) NOT NULL DEFAULT '0',  -- 星期条件
  ...
  `priority` tinyint(3) DEFAULT NULL, -- 优先级条件
  `target_client_level` varchar(50) COLLATE utf8mb4_bin DEFAULT NULL, -- 会员等级条件
  `client_created_days` int(11) DEFAULT NULL, -- 新用户限制
  PRIMARY KEY (`promo_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin

CREATE TABLE `promo_rule` (
  `promo_id` varchar(50) COLLATE utf8mb4_bin NOT NULL, -- 促销编号id
  `rule_id` varchar(50) COLLATE utf8mb4_bin NOT NULL, -- 规则id，如COUPON_20221101001R01
  `rule_seq` int(11) NOT NULL,  -- 规则序号，1 2 3之类的
  `rule_desc` varchar(200) COLLATE utf8mb4_bin DEFAULT NULL, -- 描述
  ...
  `cdtn_value` int(11) DEFAULT NULL, -- 具体的数值
...
这里有一个点需要注意的是：产品组编号保存的是当前这个促销编号中所有的产品，会存在一个表中，如上面的 Z-2022080415，到时候根据sql查询的时候，根据购物车页面的产品，查询出具体的 category_id ，然后通过promotions表的target_product_id字段 in 条件过滤出具体的数据。

![image-20230922160946226](media/images/image-20230922160946226.png)
...
获取到所有的促销活动之后，会根据活动的规则一一去匹配是否满足当前的购物活动。记住：这些活动获取到了之后会按照优先级进行排序的，从高到低，如果优先级一样则是按照促销活动的编号进行排序，从A-Z，当前项目中的优先级是**1->高，2->中，3->低**，所以上面的sql是一个升序排序的。
...
- 首先有一个定时任务，每2分钟跑一次，查询在5分钟-72小时内的订单，并且是没有本系统有支付记录的订单，根据支付方提供的api接口调用来获取订单的一个支付信息，然后更新本系统的订单的支付信息。具体是通过httpPost的方式去调用，接口方返回的是一个xml文件，然后解析xml文件，根据具体的字段来构建支付信息，并更新到本系统。

```java
CheckUnpaidSalesOrderStatusAction
```

- 支付方接收到创建订单并且支付了之后，会反过来调用系统暴露出去的一个接口，然后这个接口会根据信息更新我们系统上的订单支付信息。

```properties
POST     /:systemId/api/dataFeed                                      controllers.SSOController.paymentDataFeed(systemId: String)
```

接口代码

```java
public Result paymentDataFeed(String systemId) throws Exception {
    // form中有我们需要的是否支付成功，以及订单id等信息
    DataFeedForm form = formFactory.form(DataFeedForm.class).bindFromRequest().get();
    Logger.info("datafeed->dataFeedForm:<{}>",ReflectionToStringBuilder.toString(form));
    String merchantId = CheckUnpaidSalesOrderStatusAction.properties.getProperty("app.payDollar.merchantId");
    if (form.getMerchantId() != null && form.getMerchantId() != Long.parseLong(merchantId)) {
        Logger.info("merchantId <{}> in order <{}> is not matched app.properties's merchantId <{}>, skip!",form.getMerchantId(), form.getRef(), merchantId);
        return ok("merchantId: " + merchantId + " id not matched order's merchantId: " + form.getMerchantId()).as(MimeTypes.TEXT);
    }
    PayDollarService.getInstance().handlePaymentDataFeed(form,StringUtils.upperCase(systemId),getRealIP());
    return ok("OK").as(MimeTypes.TEXT);
}
```
