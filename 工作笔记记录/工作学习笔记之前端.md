...
![image-20230620152031080](media/images/image-20230620152031080.png)

效果如上图。

当点击下一页后，

![image-20230620152054925](media/images/image-20230620152054925.png)

到最后一页的前一页：

![image-20230620152142870](media/images/image-20230620152142870.png)

又往后一页：

![image-20230620152204182](media/images/image-20230620152204182.png)

![image-20230620152234473](media/images/image-20230620152234473.png)
...
$scope.getPagesWhenChangePage = function () {
    let newPages = [];
    if ($scope.currentPage + 4 <= $scope.totalPage - 1) {
        for (let i = 0; i <= 4; i++) {
            newPages.push($scope.currentPage + i);
        }
    } else if ($scope.currentPage + 4 > $scope.totalPage - 1) {
        let maxI = $scope.totalPage - 1 - $scope.currentPage;
        for (let i = 0; i <= maxI; i++) {
            newPages.push($scope.currentPage + i);
        }
    } else if ($scope.currentPage <= 6) {
        for (let i = 0; i < $scope.currentPage; i++) {
            newPages.push($scope.currentPage + i);
        }
    }
    return newPages;
