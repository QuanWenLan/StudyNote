...
InnoDB 和 MyISAM 这两个存储殷勤的数据如何在文件系统中存储的。
...
**这个所谓的 系统表空间 可以对应文件系统上一个或多个实际的文件，默认情况下， InnoDB 会在 数据目录 下创 建一个名为 ibdata1 （在你的数据目录下找找看有木有）、大小为 12M 的文件，这个文件就是对应的 系统表空 间 在文件系统上的表示**。这个文件时自扩展文件，也就是不够用时会自己增加文件大小。
